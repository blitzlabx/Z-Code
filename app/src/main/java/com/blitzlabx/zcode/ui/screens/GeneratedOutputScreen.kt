...
fun GeneratedOutputScreen(
    ...
    Scaffold(
        ...
        Column(
            ...
            result?.let { r ->
                Text("Z-Code", color = Color(0xFF8AA0C0))
                Card(
                ...
                Spacer(Modifier.height(16.dp))
                Text("Configuration Summary", color = Color(0xFF8AA0C0))
                Card(colors = CardDefaults.cardColors(containerColor = ZSurface), shape = RoundedCornerShape(12.dp)) {
                    Column(Modifier.padding(16.dp)) {
                        Text("• Code Type: ${r.config.codeType.displayName}", color = Color.White)
                        Text("• Hash: ${r.config.hash.displayName}", color = Color.White)
                        Text("• Z Language: ${r.config.zLanguage.displayName}", color = Color.White)
                        Text("• Mode: ${r.config.mode.displayName}", color = Color.White)
                    }
                }
                ...
                Row(Modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    OutlinedButton(onClick = {
                        val send = Intent(Intent.ACTION_SEND).apply {
                            type = "text/plain"
                            putExtra(Intent.EXTRA_TEXT, r.zCode)
                        }
                        context.startActivity(Intent.createChooser(send, "Share Z-Code"))
                    }, modifier = Modifier.weight(1f)) {
